...
/* ──────────────────────────────────────────────────────────────────────────
   ...
   The vessel GLB (400 KB, one fused mesh) explode→assembles as the band is
   scrolled: a tall section with a sticky 100lvh stage; GSAP ScrollTrigger maps
   section scroll → scrollState.progress (0 exploded → 0.6 assembled → 1 orbit/
   tag reveal), which VesselScene reads inside useFrame. Synced through the
   same Lenis-driven ticker the rest of the site uses (RigHero pattern).

   Sits in the AM cream system — the vessel reads as dark steel on warm
   sailcloth, copy is ink (NOT the retired platform band's warm-white on dark).
   Replaces the heavy offshore-platform chain (platform.glb 5.6 MB).
   ────────────────────────────────────────────────────────────────────────── */
export default function VesselShowcase() {
  ...
  if (mounted && (!useCanvas || canvasFailed)) {
    return (
      <section
        aria-label="The vessels and structures Advantage Marine services"

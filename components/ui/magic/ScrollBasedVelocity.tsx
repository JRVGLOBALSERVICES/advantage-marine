...
/**
 * ScrollBasedVelocity — Magic UI (verbatim logic, Tailwind v3 safe).
 *
 * Scrolling marquee whose speed + direction react to scroll velocity. Already
 * built on motion/react (motion ^11). No color is baked in — style the text
 * via the wrapped children (use .font-display + var(--color-ink) for the
 * Advantage Marine credential/service ticker). The impl already gates on
 * prefers-reduced-motion (constant base speed, no velocity boost), pauses when
 * off-screen, and pauses on hidden tabs.
 *
 * Compose either:
 *   <ScrollVelocityContainer> (one shared scroll signal)
 *     <ScrollVelocityRow baseVelocity={4} direction={1}>…</ScrollVelocityRow>
 *     <ScrollVelocityRow baseVelocity={4} direction={-1}>…</ScrollVelocityRow>
 *   </ScrollVelocityContainer>
 * or a standalone <ScrollVelocityRow> (creates its own scroll signal).
 */

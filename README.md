...
- **SEO infrastructure** — `app/robots.ts`, `app/sitemap.ts` (6 static routes + one URL per news post), `app/manifest.ts`; sitewide `Organization` + `LocalBusiness` + `WebSite` JSON-LD (`@id`-linked, real Gelang Patah / Johor facts) in the root layout; per-page canonical + `summary_large_image` twitter card + `en_MY` locale.
...
app/
  ...
  sitemap.ts            XML sitemap — 6 static routes + one URL per news post
